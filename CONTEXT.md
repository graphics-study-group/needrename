...
The engine is organized into layered shared libraries (DLLs), numbered from
Layer 0 (zero engine dependencies) upward. Higher layers may depend on lower
layers but not vice versa.

- **Core.dll** — Layer 1 module. Provides engine-agnostic types: GUID,
  Transform math, Delegate/Event callbacks, flagbits, SDLWindow, TimeSystem.
  Depends on Reflection.dll, SDL3, and glm. No other engine module
  dependencies. Public interface: ~8 header files under `engine/Core/`.

- **Reflection.dll** — Layer 0 module. Provides compile-time reflection code
  generation (Python/libclang) and runtime type registry with JSON
  serialization. Zero engine-module dependencies; depends only on std, glm,
  and nlohmann/json. Public interface: ~15 header files under
  `engine/Reflection/`.

  *Future*: Reflection is genuinely engine-agnostic and could be extracted
  from the `Engine` namespace into its own namespace (e.g. `Refl::` or
  `refl::`). This would clarify that it is a toolkit, not an engine subsystem.

- **engine.dll** — Layer N (top-level, to be decomposed further). Contains
  the remaining five OBJECT libraries (Asset, Framework, Physics, Render,
  UserInterface) plus MainClass. These modules are being split incrementally.

### Type Registration Init Flow

Each DLL that contains reflected types exports a `Register*Types()` function
called at startup by MainClass in dependency order:

```
MainClass::Initialize():
  1. Reflection::Initialize()      // Register basic types (int, float, glm types...)
  2. Core::RegisterCoreTypes()     // Transform
  3. RegisterAllEngineTypes()      // Asset, Framework, Physics, Render, UI types
```

The registration functions are generated by per-module `meta_*` cmake targets
and compiled into their respective DLLs. The top-level orchestrator (MainClass
in engine.dll) calls them to avoid circular DLL dependencies (Reflection.dll
cannot call registrar functions defined in Core.dll).
