...
// Dynamic viscosity
layout(constant_id = 0) const float NU = 0.7;
// Volume viscosity
layout(constant_id = 1) const float KAPPA = 0.2;
// Coefficient for corrective density field
layout(constant_id = 2) const float K = 0.2;
// Time step
layout(constant_id = 3) const float dt = 0.15;
...
// We are also ignoring OOB accesses, which might cause a small error on boundaries.
...
void main() {
    ...
    if(ubo.frame_count < 10) {
        ...
        // external source
        const float time = ubo.frame_count * dt;
