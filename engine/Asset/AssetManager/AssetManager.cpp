...
namespace Engine
{
    ...
    void AssetManager::LoadProject(std::filesystem::path path)
    {
        ...
        if (!std::filesystem::exists(asset_path))
            throw std::runtime_error("Asset path does not exist");
    ...
    void AssetManager::LoadObjResource(const std::filesystem::path &path, const std::filesystem::path &path_in_project)
    {
        ...
        GUID mesh_guid = generateGUID(m_guid_gen);
        ...
        mesh_json["type"] = "Mesh";
        mesh_json["materials"] = nlohmann::json::array();

        for (size_t s = 0; s < shapes.size(); s++)
        {
            // XXX: every face in a shape must have the same material
            size_t material_id = shapes[s].mesh.material_ids[0];
            mesh_json["materials"].push_back(GUIDToString(material_guids[material_id]));
        }

        std::filesystem::path json_path = GetAssetsDirectory() / path_in_project / (path.stem().string() + ".asset");
        std::ofstream mesh_file(json_path);
        if (mesh_file.is_open())
        {
            mesh_file << mesh_json.dump(4);
            mesh_file.close();
            m_assets[mesh_guid] = json_path.stem();
        }
        else
        {
