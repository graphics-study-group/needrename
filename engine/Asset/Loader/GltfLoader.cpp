...
namespace {
    // Replaces filesystem-invalid/control characters so generated asset names are safe to persist.
    ...
    // Ensures an asset name is unique within this import run by appending an incrementing suffix.
    ...
    // Appends one packed float attribute stream into the submesh byte buffer and records its span.
    ...
    // Builds a final .asset path under the import target folder.
    ...
    // Serializes and stores an asset through the archive/database pipeline.
    void SaveAsset(
        Engine::FileSystemDatabase &database,
    ...
    // Resolves an external texture URI to a canonical local filesystem path.
    ...
    // Lightweight non-owning byte span used to access loaded glTF buffer payloads.
    struct ByteView {
        const std::byte *data{nullptr};
        size_t size{0};

        // True when the view points to non-empty data.
        bool IsValid() const {
            return data != nullptr && size > 0;
        }
    };

    // Extracts raw bytes from fastgltf data source variants that keep data in memory.
    std::optional<ByteView> GetDataView(const fastgltf::DataSource &source) {
        return std::visit(
            fastgltf::visitor{
                // Sources that do not carry directly readable in-memory bytes.
                ...
                // Sources that expose direct byte vectors.
    ...
    // Bundles all material/texture artifacts produced during material translation.
    ...
    // Builds a runtime texture asset from one glTF image source (external, embedded, or buffer view).
    std::optional<Engine::AssetRef> BuildTextureAssetFromImage(
        ...
        image_texture->m_name =
            MakeUniqueAssetName(model_name + "_image_" + std::to_string(image_index), name_counters);

        // Tracks whether any source variant was successfully decoded.
        bool loaded = false;
        ...
        std::visit(
            fastgltf::visitor{
                ...
                [&](const fastgltf::sources::URI &uri_source) {
                    // External image file path relative to model.
                ...
                [&](const fastgltf::sources::ByteView &byte_view_source) {
                    // Embedded bytes (byte-view storage).
                ...
                [&](const fastgltf::sources::BufferView &buffer_view_source) {
                    // Image payload stored inside a glTF bufferView.
                ...
                [&](const fastgltf::sources::CustomBuffer &) {
                    // Custom callbacks are not wired in current loader path.
                    SDL_LogWarn(
                        SDL_LOG_CATEGORY_APPLICATION,
                ...
                [&](const fastgltf::sources::Fallback &) {
                    // Fallback means no concrete source payload can be consumed here.
                    SDL_LogWarn(
                        SDL_LOG_CATEGORY_APPLICATION,
    ...
    // Builds required material assets and dependent texture assets for the imported meshes.
    MaterialBuildOutput BuildMaterials(
        ...
        // Caches textures by image index to avoid creating duplicate texture assets.
        ...
        default_material->m_library =
            db.GetNewAssetRef(Engine::AssetPath(db, std::filesystem::path("~/material_libraries/PBRLibrary.asset")));
        ...
        // Creates a fallback solid-color texture asset when a texture map is missing.
        auto create_solid_color_texture = [&](const glm::vec4 &color,
                                              const std::string &name_suffix) -> Engine::AssetRef {
        ...
        // Validates glTF texture info, enforces UV0 usage, loads/caches texture, and outputs material property.
        ...
        // Material loop: translate each referenced glTF material into runtime PBR material properties.
        for (size_t material_index : required_material_indices) {
            if (material_index >= asset.materials.size()) {
                // Broken material index: map to default material to keep import robust.
            ...
            if (!try_load_texture_ref(source_material.pbrData.baseColorTexture, albedo_prop, "baseColorTexture")) {
                // If base color map is absent, use default or bake baseColorFactor into a solid texture.
            ...
            // MRAO map supports metallic/roughness path; AO channel merge from separate map is unsupported.
            ...
            // Record unsupported maps/extensions explicitly so authored content issues are visible.
