...
namespace Engine {
    ...
    void ObjLoader::LoadObjResource(const std::filesystem::path &path, const std::filesystem::path &path_in_project) {
        ...
        auto m_mesh_asset = am->CreateAsset<MeshAsset>();
        m_mesh_asset->m_name = path.stem().string();
        LoadMeshAssetFromTinyObj(*m_mesh_asset, attrib, shapes);

        std::vector<MaterialAsset *> m_material_assets;
        std::vector<Image2DTextureAsset *> m_texture_assets;
        ...
        for (const auto &material : materials) {
            ...
            if (it == material_name_to_index.end()) {
                ...
                m_material_assets.push_back(m_material_asset.get());
        ...
        mesh_component.m_mesh_asset = AssetRef(m_mesh_asset);
        auto submesh_count = m_mesh_asset->m_submeshes.size();
        for (size_t i = 0; i < submesh_count; i++) {
            if (i < materials.size()) {
                mesh_component.m_material_assets.push_back(
                    AssetRef(m_material_assets[material_name_to_index[materials[i].name]])
                );
            } else {
        ...
        database->SaveArchive(
            archive, AssetPath(*database, path_in_project / ("GO_" + m_mesh_asset->m_name + ".asset"))
        );
    }
    ...
    void ObjLoader::LoadMaterialAssetFromTinyObj(
        ...
        case 2: // Blinn-Phong
        {
            ...
            if (!material.diffuse_texname.empty()) {
                ...
                material_asset.m_properties["base_tex"] = {AssetRef(texture), MaterialProperty::Type::Texture};
            }
            break;
        }
        default: // unknown model, load every property
        {
            ...
            if (!material.ambient_texname.empty()) {
                ...
                material_asset.m_properties["ambient_texture"] = {
                    AssetRef(texture.get()), MaterialProperty::Type::Texture
                };
            }
            if (!material.diffuse_texname.empty()) {
                auto texture = std::make_shared<Image2DTextureAsset>();
