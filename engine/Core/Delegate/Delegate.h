...
namespace Engine
{
    ...
    public:
        ...
        Delegate(std::weak_ptr<void> object, FunctionType function)
            : m_object(object), m_function(function) {}
        template <typename T>
        Delegate(std::shared_ptr<T> object, void (T::*method)(Args...))
            : m_object(object)
        {
            auto ptr = object.get();
            m_function = [ptr, method](Args... args)
            {
                (ptr->*method)(args...);
            };
        }
        template <typename T>
        Delegate(std::weak_ptr<T> object, void (T::*method)(Args...))
            : m_object(object)
        {
