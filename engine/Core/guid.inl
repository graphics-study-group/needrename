...
inline constexpr uint8_t unhex(char ch)
{
    if ((ch >= '0') && (ch <= '9'))
        ...
        throw std::invalid_argument("Invalid UUID character");
}

} // namespace Internals
//! @endcond

inline constexpr GUID::GUID(const char* uuid, size_t size) : _data{}
{
    char v1 = 0;
    char v2 = 0;
    bool pack = false;
...
inline void GUID::swap(GUID& uuid) noexcept
{
    using std::swap;
    swap(_data, uuid._data);
}

inline void swap(GUID& uuid1, GUID& uuid2) noexcept
{
    uuid1.swap(uuid2);
}
...
template <>
struct fmt::formatter<Engine::UUID> : formatter<std::string_view>
{
    template <typename FormatContext>
    auto format(const Engine::UUID& value, FormatContext& ctx) const
    {
...
{
    typedef Engine::GUID argument_type;
