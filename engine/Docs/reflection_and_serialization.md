...
5. **Enum Serialization**
    ...
    -   **Serialization Process for Enums:**
        ...
        -   If it is not reflectable, the system attempts to call the template `to_string<EnumType>` function.
            -   If this template function was instantiated by users (eg: X-MACRO), it serializes the enum to user defined format.
            -   If the template function is not instantiated, the system falls back to serializing the enum as its underlying integer value.
