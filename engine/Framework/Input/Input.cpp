...
namespace Engine {
    ...
    float Input::InputAxis::GetRawValue() const {
        float ret = m_value;
        if (m_invert) ret = -ret;
        if (m_dead > 0.0f && std::abs(ret) < m_dead) ret = 0.0f;
        return ret;
    }
    ...
    void Input::InputAxis::UpdateSmoothedValue(float delta_time) {
        ...
        if (m_value == 0.0f) m_smoothed_value += dir * m_gravity * dt;
        else m_smoothed_value += dir * m_sensitivity * dt;
    ...
    void Input::ButtonAxis::ProcessEvent(SDL_Event *event) {
        if ((event->type == SDL_EVENT_KEY_DOWN || event->type == SDL_EVENT_KEY_UP) && event->key.repeat == false) {
            if (k_name_code_map.find(m_positive) != k_name_code_map.end()
                && event->key.scancode == k_name_code_map.at(m_positive))
                m_value += event->key.down ? 1.0f : -1.0f;
            if (k_name_code_map.find(m_negative) != k_name_code_map.end()
            ...
            if (k_name_code_map.find(m_negative) != k_name_code_map.end()
                && event->gbutton.button == k_name_code_map.at(m_negative))
                m_value += event->gbutton.down ? -1.0f : 1.0f;
            m_value = std::clamp(m_value, -1.0f, 1.0f);
        }
    }
    ...
    void Input::MotionAxis::ProcessEvent(SDL_Event *event) {
        if (event->type == SDL_EVENT_MOUSE_MOTION) {
            if (m_axis == "x") m_sum_one_frame += event->motion.xrel;
            else if (m_axis == "y") m_sum_one_frame += event->motion.yrel;
        }
        if (event->type == SDL_EVENT_MOUSE_WHEEL) {
            if (m_axis == "x") m_sum_one_frame += event->wheel.x;
            else if (m_axis == "y") m_sum_one_frame += event->wheel.y;
        }
    }

    void Input::MotionAxis::UpdateSmoothedValue(float delta_time) {
        m_value = m_sum_one_frame * m_motion_sensitivity;
        m_sum_one_frame = 0.0f;
        InputAxis::UpdateSmoothedValue(delta_time);
    }
    ...
    void Input::GamepadAxis::ProcessEvent(SDL_Event *event) {
        if (event->type == SDL_EVENT_GAMEPAD_AXIS_MOTION) {
            if (k_name_code_map.find(m_axis) != k_name_code_map.end()
                && event->gaxis.axis == k_name_code_map.at(m_axis))
    ...
    void Input::Update(float delta_time) {
        for (auto &axis : m_axes) {
            axis->UpdateSmoothedValue(delta_time);
        }
    }
    ...
    float Input::GetAxis(const std::string &axis_name) const {
        ...
        for (auto &axis : m_axes) {
            if (axis->m_name == axis_name) {
                ...
                if (std::abs(temp) > std::abs(result)) result = temp;
            }
        }
        return result;
    }
