...
namespace Engine {
    ...
    public:
        ...
        public:
            ...
            float m_gravity =
                3.0f; // Speed in units per second that the axis falls toward neutral when no input is present.
            float m_dead = 0.01f; // How far the user needs to move an analog stick before your application registers
                                  // the movement. At runtime, input from all analog devices that falls within this
                                  // range will be considered null.
            float m_sensitivity = 3.0f; // Speed in units per second that the axis will move toward the target value.
                                        // This is for digital devices only.
            bool m_snap = false;        // If enabled, the axis value will reset to zero when pressing a button that
            ...
            virtual void ProcessEvent(SDL_Event *event);
            virtual void UpdateSmoothedValue(float delta_time);
        };
        ...
        public:
            ...
            virtual void ProcessEvent(SDL_Event *event) override;
            virtual void UpdateSmoothedValue(float delta_time) override;
        };
        ...
        void AddAxis(const AxisType &axis) {
            m_axes.push_back(std::make_shared<AxisType>(axis));
        }
