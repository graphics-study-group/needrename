...
namespace Engine {
    ...
    struct ParallelScan::Impl {
        ...
        void RecordScanPass(
            ...
            cb.BindComputeStage(*scan_stage);
            cb.BindComputeResource(*scan_binding);
        ...
        void RecordOffsetPass(
            ...
            cb.BindComputeStage(*offset_stage);
            cb.BindComputeResource(*offset_binding);
            cb.DispatchCompute(num_workgroups, 1, 1);
        }
        ...
        void RecordScanInternal(
            ...
            if (elem_count <= kMaxSingleLevel) {
                ...
                RecordScanPass(cb, data_input_buf, data_output_buf, block_sums_buf, param, 1u);
                return;
            }
            ...
            // --- Pass 1: scan blocks, write per-block sums ---
            {
                ...
                RecordScanPass(cb, data_input_buf, data_output_buf, block_sums_buf, param, num_blocks);
            }
            ...
            if (num_blocks <= kMaxSingleLevel) {
                ...
                RecordScanPass(cb, block_sums_buf, block_sums_buf, block_sums_buf, param, 1u);
            } else {
                RecordScanInternal(
                    cb,
                    block_sums_buf,
            ...
            // --- Pass 3: add prefix-summed block offsets back to data ---
            {
                ...
                RecordOffsetPass(cb, data_output_buf, data_output_buf, block_sums_buf, param, num_blocks);
            }
        }
    };
