...
namespace Engine
{
    namespace Reflection
    {
        ...
        void Type::AddField(const std::shared_ptr<const Type> field_type, const std::string &name, const WrapperFieldFunc &field, const WrapperConstFieldFunc &const_field)
        {
            m_fields[name] = std::shared_ptr<const Field>(new Field(name, shared_from_this(), field_type, field, const_field));
        }
        ...
        VectorType::VectorType(std::shared_ptr<const Type> element_type)
            : Type(std::string("std::vector<") + element_type->m_name + ">"), m_element_type(element_type)
        {
            m_specialization = StdVector;
        }

        std::shared_ptr<const Type> VectorType::GetElementType() const
        {
