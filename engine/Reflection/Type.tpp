...
namespace Engine
{
    namespace Reflection
    {
        ...
        void Type::AddConstructor(const WrapperMemberFunc &func)
        {
            ...
            m_methods[mangled_name] = std::shared_ptr<const Method>(new Method(mangled_name, func, shared_from_this(), true));
        }

        template <typename... Args>
        void Type::AddMethod(const std::string &name, const WrapperMemberFunc &func, std::shared_ptr<const Type> return_type, bool is_const)
        {
            std::string mangled_name = name + GetMangledName<Args...>();
            m_methods[mangled_name] = std::shared_ptr<const Method>(new Method(mangled_name, func, return_type, is_const, true));
        }
