...
namespace Engine {
    ...
    void AllocatedImage2D::Create(uint32_t width, uint32_t height, ImageUtils::ImageType type, ImageUtils::ImageFormat format, uint32_t mip) {
        auto system = m_system.lock();

        m_allocated_memory = system->GetAllocatorState().AllocateImageUnique(type, VkExtent3D{width, height, 1}, static_cast<VkFormat>(ImageUtils::GetVkFormat(format)));
