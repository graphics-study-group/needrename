...
namespace Engine {
    ...
    void TransferCommandBuffer::GenerateMipmaps(const Texture &img, AccessHelper::ImageAccessType previousAccess) {
        ...
        assert(desc.dimensions == 2 && desc.array_layers == 1
               && "Mipmap is only supported for 2D textures with one layer.");

        auto previousAccessScope = AccessHelper::GetAccessScope(previousAccess);
        std::array<vk::ImageMemoryBarrier2, 2> barriers = {
            vk::ImageMemoryBarrier2{// This is a very conservative barrier.
            ...
            vk::ImageMemoryBarrier2{std::get<0>(previousAccessScope),
        ...
        for (uint32_t level = 1; level < total_levels; level++) {
            cb.blitImage(img.GetImage(),
                         vk::ImageLayout::eTransferSrcOptimal,
                         img.GetImage(),
                         vk::ImageLayout::eTransferDstOptimal,
                         {vk::ImageBlit{vk::ImageSubresourceLayers{vk::ImageAspectFlagBits::eColor, level - 1, 0, 1},
