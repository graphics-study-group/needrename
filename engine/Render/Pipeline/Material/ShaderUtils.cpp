...
namespace Engine
{
    namespace ShaderUtils
    {
        ...
        ProcessDescriptors(
            ...
            for (uint32_t i_binding = 0; i_binding < refl_set.binding_count; ++i_binding)
            {
                ...
                layout_binding.stageFlags = static_cast<vk::ShaderStageFlagBits>(
                    static_cast<VkShaderStageFlagBits>(shader.GetShaderStage())
                );
                ...
                switch(layout_binding.descriptorType) {
                    ...
                    case vk::DescriptorType::eUniformBuffer:
                        ...
                        vars.push_back(DesciptorVariableData{
                            .set = refl_set.set,
        ...
        ReflectedDataCollection ReflectSpirvData(const std::vector<uint32_t> &spirv_code)
        {
            ...
            assert((shaderModule.GetShaderStage() & VK_SHADER_STAGE_ALL_GRAPHICS) && "The shader provided is not a graphic pipeline shader.");
            ...
            for (size_t i_set = 0; i_set < descriptor_sets.size(); ++i_set)
            {
                ...
                ProcessDescriptors(
                    shaderModule, 
                    refl_set, 
                    reflected.desc.names, reflected.desc.vars, 
                    reflected.inblock.names, reflected.inblock.vars, 
                    layout->bindings
                );
        ...
        {
            spv_reflect::ShaderModule shaderModule{spirv_code};
            assert((shaderModule.GetShaderStage() & VK_SHADER_STAGE_COMPUTE_BIT) && "The shader provided is not a compute shader.");
