...
namespace Engine {
    namespace ShaderUtils {
        std::vector<std::pair<std::string, InBlockVariableData>> EnumerateInBlockVariables(
            ...
            for (uint32_t i = 0; i < desc_binding.block.member_count; i++) {
                ...
                if (type_desc.type_flags & SPV_REFLECT_TYPE_FLAG_VECTOR) {
                    assert((type_desc.type_flags & SPV_REFLECT_TYPE_FLAG_FLOAT)
                           && "Only vec4 vector type is supported.");
                    assert(type_desc.traits.numeric.vector.component_count == 4
                           && "Only vec4 vector type is supported.");
                    ...
                    assert(type_desc.traits.numeric.scalar.width == 32
                           && "Only 4-byte single precision float is supported");
                    ...
                    assert(type_desc.traits.numeric.scalar.signedness == 1
        ...
        void ProcessDescriptors(const spv_reflect::ShaderModule &shader,
            ...
            for (uint32_t i_binding = 0; i_binding < refl_set.binding_count; ++i_binding) {
                ...
                case vk::DescriptorType::eUniformBuffer:
                    ...
                    vars.push_back(DesciptorVariableData{.set = refl_set.set,
        ...
        ReflectedDataCollection ReflectSpirvData(const std::vector<uint32_t> &spirv_code) {
            ...
            assert((shaderModule.GetShaderStage() & VK_SHADER_STAGE_ALL_GRAPHICS)
            ...
            for (size_t i_set = 0; i_set < descriptor_sets.size(); ++i_set) {
                ...
                ProcessDescriptors(shaderModule,
                                   refl_set,
        ...
        ReflectedDataCollectionCompute ReflectSpirvDataCompute(const std::vector<uint32_t> &spirv_code) {
            spv_reflect::ShaderModule shaderModule{spirv_code};
            assert((shaderModule.GetShaderStage() & VK_SHADER_STAGE_COMPUTE_BIT)
            ...
            assert(refl_set.set == 0
                   && "Only exactly one descriptor set numbered zero is supported for compute shader.");
