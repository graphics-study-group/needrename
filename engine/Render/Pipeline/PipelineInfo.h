...
namespace Engine {
    ...
    namespace PipelineInfo {
        ...
        struct InstancedPassInfo {
            // FIXME: We are only allocating one buffer for multiple frames-in-flight. This might lead to
         ...
         * 
         * The
         * layout for the image is selected based on its type.
         * If the type is `Texture`, then the layout is
         * `ReadOnlyOptimal`.
