...
namespace Engine::PipelineUtils {
     ...
     */
    constexpr vk::ShaderStageFlagBits ToVulkanShaderStageFlagBits(ShaderAsset::ShaderType type) {
    ...
    constexpr vk::PipelineRasterizationStateCreateInfo ToVulkanRasterizationStateCreateInfo(
        ...
        is_depth_bias_enabled =
            is_depth_bias_enabled && (prop.depth_bias_slope != 0.0f && prop.depth_bias_slope != -0.0f);
    ...
    inline vk::SpecializationInfo FillSpecializationInfo(
        ...
        if (!spec_vars.empty()) {
            ...
            for (const auto &[k, v] : spec_vars) {
                ...
                sme.push_back(vk::SpecializationMapEntry{k, current_offset, size});
                current_offset += size;
            }
