...
namespace Engine {
    ...
    public:
        ...
        int32_t ImportExternalResource(
            const RenderTargetTexture &texture, MemoryAccessTypeImageBits prev_access = MemoryAccessTypeImageBits::None
        );
         ...
         */
        void UseImage(int32_t texture_handle, MemoryAccessTypeImageBits access);

        /**
         * @brief Mark a buffer to be used in the following pass.
         */
        void UseBuffer(int32_t buffer_handle, MemoryAccessTypeBuffer access);
         ...
         */
        void RecordRasterizerPassWithoutRT(std::function<void(GraphicsCommandBuffer &, const RenderGraph &)> pass);
        ...
        std::unique_ptr<RenderGraph> BuildDefaultRenderGraph(
            uint32_t width, uint32_t height, GUISystem *gui_system = nullptr
        );
    };
} // namespace Engine
