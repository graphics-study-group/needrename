    ...
    namespace RenderGraphImpl {
        ...
        struct Synchronization {
            ...
            std::function<void(vk::CommandBuffer)> GetBarrierCommand() {
                return [mb = std::move(memory_barriers),
                        bb = std::move(buffer_barriers),
                        ib = std::move(image_barriers)](vk::CommandBuffer cb) -> void {
                    cb.pipelineBarrier2(vk::DependencyInfo{vk::DependencyFlags{}, mb, bb, ib});
                };
            }
            ...
            std::function<void(vk::CommandBuffer)> GetBarrierCommandCopy() {
                return [mb = memory_barriers, bb = buffer_barriers, ib = image_barriers](vk::CommandBuffer cb) -> void {
                    cb.pipelineBarrier2(vk::DependencyInfo{vk::DependencyFlags{}, mb, bb, ib});
                };
            }
        };
