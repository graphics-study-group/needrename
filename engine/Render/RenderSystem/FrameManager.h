 ...
 * is ready
             ...
             * 
             * @warning Must be called before
             * either `CompositeToFramebufferAndPresent` or `CompositeToImage`, once each frame.
             *
 ...
 * The
             * image must be in Color Attachment Optimal layout, which should be guaranteed so long as
             *
             * this method is called immediately after a draw call to copy a color attachment to the framebuffer.
 * 

             * * The method transits the image to Transfer Source Layout and the framebuffer to Transfer Destination
             * Layout,
             * record a image copy command (not blitting command, so resizing is not possible),
             * and transits the image
            ...
            void StageCopyComposition(
                vk::Image image, vk::Extent2D extent, vk::Offset2D offsetSrc = {0, 0}, vk::Offset2D offsetDst = {0, 0}
            );
 ...
 * this
             * method is called immediately after a draw call to copy a color attachment to the framebuffer.
 * 
 * The
             * method transits the image to Transfer Source Layout and the framebuffer to Transfer Destination Layout,
 ...
 * 
             ...
             * @brief Stage a composition by copy of the given image to an undetermined image.
 * This
             * overload executes the copy with zero offset and the swapchain extent.
             * See the complete
            ...
            /**
             * @brief Execute staged composition operation to the framebuffer, and equeue a present
             * command.
             * 
             * Record and submit the command buffer for frame image copying.

             * * 
             * The command buffer used for copying is distinct from the render command buffer, but is
             * submitted into the
             * same graphic queue. Execution is halted before the semaphore marking
             * the completion of the render command
             * buffer is signaled. Only after its execution is
             * completed, semaphores for presenting and rendering for the
             * next frame will be signaled,
             * which means only exactly one render command buffer can be executed at the same time.
             * A
             * fence is signaled after this command buffer has finished execution. The same fence is used to control
             * render
             * command buffer acquisition for this frame, c.f. `StartFrame()`.
             * 

             * * @warning This should be the last method to be called each frame, and it will progress the internal
             * state machine.
             * Either `CompositeToFramebufferAndPresent()` or `CompositeToImage()` must be
             * called exactly once each frame.
             * Non-standard call-sites are likely to result in deadlocks
             * and vulkan device losses.
             * 
             * @return whether the swapchain needs to be
             * recreated after presenting.
             */
            [[nodiscard]]
             ...
             * The image is
             * guaranteed to be in a layout suitable for sampling after the composite.
             * 
             * If
             * timeout is not zero, this method will be synchronous and will wait for the copy
             * command to
             * be completed until timed out.
             * If the method is called in an asynchronous way, the fence
             * returned can be used
             * to check its completion.
             * 
             * @warning This
             * should be the last method to be called each frame, and it will progress the internal state machine.

             * * Either `CompositeToFramebufferAndPresent()` or `CompositeToImage()` must be called exactly once each
             * frame.
             * Non-standard call-sites are likely to result in deadlocks and vulkan device
             * losses.
             */
