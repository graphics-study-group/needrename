...
int main() {
    ...
    std::cout << "middle_member.middle_value: "
              << topLevelVar.GetMemberRecursively("middle_member.middle_value").Get<int>() << std::endl;
    ...
    std::cout << "middle_member.nested_member.level1_value: "
              << topLevelVar.GetMemberRecursively("middle_member.nested_member.level1_value").Get<int>() << std::endl;

    // Verify the same value is accessible via regular GetMember chain
    assert(
        topLevelVar.GetMember("middle_member").GetMember("nested_member").GetMember("level1_value").Get<int>() == 777
    );
    ...
    assert(
        topLevelVar.GetMemberRecursively("middle_member.nested_member.level1_name").Get<std::string>()
        == "recursive_test"
    );
    std::cout << "middle_member.nested_member.level1_name: "
              << topLevelVar.GetMemberRecursively("middle_member.nested_member.level1_name").Get<std::string>()
              << std::endl;

    // Test accessing string via regular GetMember for comparison
    assert(
        topLevelVar.GetMember("middle_member").GetMember("nested_member").GetMember("level1_name").Get<std::string>()
        == "recursive_test"
    );
