...
int main() {
    ...
    thd.Resume();
    thd.GetServiceQueue().push(
        std::make_unique<RenderTasks::RenderTaskBuildActiveRenderGraph>(std::move(rgb))
    );
    thd.GetServiceQueue().push(
        std::make_unique<RenderTasks::RenderTaskRenderOneFrameWithActiveGraph>(mbuffer)
    );
