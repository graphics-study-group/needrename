...
The constructor SHALL accept a `RenderSystem&` and a `max_collision_pairs` count. No GPU resources SHALL be allocated until the first call to `Step()`.
...
- `collision_normals`: `vec4` buffer storing the contact normal in world space (w = penetration depth)
...
- **AND** writes each point as a separate entry with its own contact positions
...
2. Generate 6 perturbed directions: for each 60° step around the normal, create a direction tilted 2° from the contact plane using `cos(angle)*u + sin(angle)*v + tan(2°)*normal`
3. For each perturbed direction, query `support()` on both shapes, project the world-space points onto the 2D contact plane (dropping the normal component)
4. Apply Sutherland-Hodgman clipping: clip the projected polygon of shape A by the edges of the projected polygon of shape B
5. If the clipped polygon has > 4 vertices, apply Rotating Calipers to select the 4 vertices forming the maximum-area quadrilateral
6. Un-project each selected vertex back to world space on the contact plane, then offset along the contact normal by ±half_depth to produce contact points on each shape's surface

Each un-projected vertex SHALL produce a separate result entry in the output buffers, with its own `contact_point_a` and `contact_point_b` values. Up to 4 entries per collision pair SHALL be written.

#### Scenario: Face-to-face contact produces 4 contact points
- **WHEN** two boxes rest face-to-face
- **THEN** the manifold produces up to 4 contact points
- **AND** each contact point on A has a corresponding point on B separated by penetration depth along the normal
...
- **AND** the points are valid world-space positions on both shapes
