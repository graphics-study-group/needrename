...
8. Atomically accumulate linear and angular position deltas via `atomicCompSwap` loop on int SSBOs
...
Per-constraint compute dispatches SHALL use a fixed workgroup count calculated from `max_collision_pairs * 4`. Threads with index >= `collision_count` SHALL return immediately.
