...
**Reason**: Buffer import via `RenderGraphBuilder::ImportExternalResource` is removed. Buffers are bound directly via `ShaderResourceBinding::BindBuffer`.
**Migration**: Buffer binding happens in `PreGPUStep` when creating the resource binding. No import step needed at dispatch time.
