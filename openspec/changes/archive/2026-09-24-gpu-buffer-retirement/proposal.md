...
- **Allocation-layer cut (no buffer API change).** `BufferAllocation` gains an optional hand-off to the epoch tracker; its destructor hands the allocation to the tracker instead of destroying it. `ComputeBuffer`, `DeviceBuffer`, `IndexedBuffer`, all five `EnsureBuffer` copies and every algorithm contract are untouched.
- `AllocatorState` gains a setter that installs the tracker, keeping its pinned `AllocatorState(DeviceInterface &)` constructor intact. There is no interface between the allocator and the tracker: the tracker is the only recipient of retired allocations.
...
- `rhi-module`: the module inventory gains the epoch tracker — the sole recipient of retired buffer allocations — and the shared-ownership buffer factory.
- `rhi-directory-structure`: the tracker is assigned to its subdirectory.
...
- `engine/Rhi/Device/` — the `BufferAllocation` hand-off and the `AllocatorState` setter that installs the tracker; `DeviceContext` owns the tracker.
- `engine/Rhi/Buffer/` — `ComputeBuffer::CreateShared`.
