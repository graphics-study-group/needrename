...
**Reason**: Filter resolution moves from `PhysicsScene` to `PhysicsAdaptor`. `PhysicsScene` no longer handles ObjectHandles or ComponentHandles — it only receives resolved shape index pairs via `SetShapeFilters`.

**Migration**: No migration needed. The `PhysicsAdaptor::Flush` method now performs both resolution and symmetry derivation before calling `SetShapeFilters`.

### Requirement: Filter data cleaned on shape unregistration

**Reason**: Full rebuild via `SetShapeFilters` makes explicit cleanup unnecessary — non-alive shapes are excluded during each Flush. Stale entries in `m_filter_map` that reference destroyed components are skipped with a warning when they fail to resolve.

**Migration**: `UnregisterCollisionShape` continues to set `m_shape_alive[i] = 0`. Filter data for stale entries is naturally excluded by the next `SetShapeFilters` call. Full cleanup of `m_filter_map` is deferred to a future refactor with a TODO comment.

## RENAMED Requirements

FROM: `PhysicsScene stores unresolved filter ObjectHandles`
TO: `PhysicsAdaptor manages filter declarations`

FROM: `Symmetric filter data on CPU`
TO: `Symmetric filter data via greedy allocation`

FROM: `GPU binary search filter check`
TO: `GPU linear scan filter check`

FROM: `GPU filter data buffers`
TO: `GPU fixed-stride filter data buffer`
