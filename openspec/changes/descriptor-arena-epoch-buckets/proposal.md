...
- **The arena resolves a layout description to the shared layout object once, and callers hand it back that object.** A layout is what the cache key needs and what a pipeline layout is built over, so the consumer resolves it through the arena when it builds its pipeline and passes the same object to the arena on every acquisition — instead of the per-frame re-derivation a description would cost.
...
- **BREAKING:** `ShaderResourceBinding::GetDescriptorSet` loses its `pool` parameter and takes the arena-resolved descriptor-set layout instead of deriving it, and `ComputeResourceBinding::GetDescriptorSet` / `MaterialInstance::GetDescriptor` are deleted along with their handle arrays. `ComputeStage::GetDescriptorPool` and `MaterialTemplate::GetDescriptorPool` are removed. `ComputeStage`, `MaterialTemplate`/`MaterialLibrary`, `SceneDataManager` and `CameraManager` all stop owning a descriptor pool and resolve their descriptor-set layout through the arena, and `MaterialTemplate` computes its "has per-material data" fact once from the reflected layout instead of reading it off the pool.
- **`ComputeStage` resolves its descriptor set layout through the device descriptor arena**, as `MaterialLibrary` already resolves its own, so the pipeline's layout and the layout a set is allocated against are the same object — which also removes a hazard the previous change recorded rather than fixed.
...
**Code**
- `engine/Rhi/Resource/` — new `DescriptorArena.*`; `ImmutableResourceCache` unchanged but now the layout source the arena resolves against.
- `engine/Rhi/Pipeline/` — `ComputeStage.*` (pool removal, layout resolved through the arena), `ComputeResourceBinding.*` (arena acquisition over the stage's layout, returned set, handle array deleted), `ShaderResourceBinding.*` (becomes the name resolver; no own cache, no pool parameter, no layout derivation).
...
- `engine/Render/Pipeline/Material/` — `MaterialInstance.cpp` and `.h` call sites; `MaterialTemplate.*` and `MaterialLibrary.cpp` lose the per-template pool and its naming, carry the arena-resolved layout, and the pool-derived "has material data" fact becomes a cached layout-derived boolean.
...
**API**
- Breaking: `ShaderResourceBinding::GetDescriptorSet` (the `pool` parameter is replaced by the arena-resolved descriptor-set layout), `ComputeResourceBinding::GetDescriptorSet`, `MaterialInstance::GetDescriptor`, `ComputeStage::GetDescriptorPool`, `MaterialTemplate::GetDescriptorPool`, the `layout` parameter of both arena acquisition services, and the return type of both `UpdateGPUInfo` overloads.
- Additive: `DescriptorArena` (including `ResolveLayout`), `MaterialTemplate::GetDescriptorSetLayout`, `DeviceContext::WaitForIdle`.
