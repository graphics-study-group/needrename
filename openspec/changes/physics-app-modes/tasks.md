## 1. Engine: generic image-to-buffer copy utility

- [x] 1.1 Implement pure function `RecordCopyImageToBuffer(vk::CommandBuffer cb, const Rhi::Texture&, const Rhi::DeviceBuffer& dst, Rhi::MemoryAccessTypeImageBits last_access)` in `MemoryAccessHelper.hpp` (or sibling util): pre-barrier `GetImageLayout(last_access) → eTransferSrcOptimal` (access `GetAccessFlags(last_access) → eTransferRead`), `copyImageToBuffer` with extent from `GetTextureDescription()` (mip 0, color aspect), post-barrier back to `GetImageLayout(last_access)`. Mirror the `SwapchainPresentProvider::RecordCopyCommand` barrier recipe.
...
## 3. Engine: headless extent flows from StartupOptions

- [x] 3.1 `RenderSystem` constructor gains a defaulted `vk::Extent2D headless_extent = {1920, 1080}` parameter, stored in `pimpl`.
- [x] 3.2 `RenderSystem::Create` headless branch replaces the hard-coded `{1920, 1080}` with the constructor-provided extent.
- [x] 3.3 `MainClass::Initialize` passes `StartupOptions::resol_x/resol_y` as the `headless_extent` to the `RenderSystem` constructor.
...
- [x] 4.3 `PrepareCopy` records `copyImageToBuffer` (reusing/equivalently the 1.1 utility) into the buffer for `image_index` and returns the recorded command buffer; source layout derived via `GetImageLayout(last_access)`.
...
## 5. PhysicsApp: AppMode and mode-1 (Headless) simplification

- [x] 5.1 Add `enum class AppMode { Headless, Offscreen, Windowed }` and `CreateInfo::mode` (default `Windowed`) in `PhysicsApp.h`.
- [x] 5.2 `PhysicsApp::Create` branches by mode: `Headless`/`Offscreen` set `StartupOptions.headless = true`; `Headless` skips camera GameObject/CameraComponent/CameraController/SetActiveCamera/initial `SetCameraPose`.
...
- [x] 5.4 `CommitScene` in `Headless` mode skips `BuildDefaultRenderGraph` and the model-matrices bridge.
- [x] 5.5 Mode-specific throws: in `Headless` mode `RenderNextFrame`, `GetRenderOutput`, `SetRenderReadbackEnabled(true)`, `AddDirectionalLight`, and `SetCameraPose` throw `std::logic_error`.
- [x] 5.6 `ShouldQuit` returns `false` in `Headless`/`Offscreen` modes (no SDL_QUIT source).
...
- [x] 7.2 `RenderNextFrame`: when enabled (and extent matches), record `RecordCopyImageToBuffer` into the main CB after `RecordAllPasses` and record the current frame id.
...
- [x] 7.4 `RenderNextFrame` mode branch: `Headless` throws; `Offscreen` skips the input section (SDL polling, input update, SPACE toggle); `Windowed` unchanged.

## 8. Tests and example migration

- [x] 8.1 Remove `example/physics_example` (main.cpp, its CMake registration, and the directory).
- [x] 8.2 Add `test/physics_app_headless_test.cpp`: mode-1 physics readback (initial state legal before first step, mapping length/unique, step advances position under gravity), negative throws (RenderNextFrame/GetRenderOutput/SetRenderReadbackEnabled(true)/AddDirectionalLight/SetCameraPose).
