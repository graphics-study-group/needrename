# physics-constraint-serialization

## Purpose

Provide custom archive serialization for `PhysicsConstraintComponent` so its `std::vector<std::variant<FixedJointDef, HingeJointDef>>` joint list survives scene save/load round-trips.
