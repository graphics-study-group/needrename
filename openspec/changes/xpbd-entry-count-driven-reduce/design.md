...
Points where the plan above did not survive contact with the code. The full list is in `tasks.md` under "Implementation deviations".

- **The migration plan's step 2 and step 3 cannot both be true.** Step 3 claims the tree stays green with only the headless algorithm tests updated, but `SumByKey`/`RadixSort` and `XPBDGpuSolver` share the `EnginePhysics` library, so the geometry-free change breaks the build until the solver's call sites are adapted. What was kept separate instead is the *behaviour*: one commit carries the algorithm API plus the minimum solver adaptation (every count still equals its capacity), and the next carries the GPU-published contact count, the exact joint counts and the counted clears. Step 2 is also a compile-level milestone rather than a runtime-complete state, because `sum_by_key.comp` is called by `SumByKey::Record`, which does not bind `EntryCount` until the algorithm commit.
- **`SpatialHashBroadDetector` was an unlisted caller of the changed API.** It constructed `RadixSort` with `max_output_pair_count` and used `GetMaxElemCount()` to detect a stale instance; it now creates the instance once and relies on `Record`'s per-call buffer-size check.
- **The capacity-change fixture needs two things the plan did not anticipate.** `PhysicsScene::SyncGpuBuffers` re-uploads every host column, so a structural change (a shape count change) must fold the GPU state back into those columns first, or the simulation resets; and `PhysicsSystem::PreGPUStep`/`GPUStep` advance every scene in the system, so two scenes that must stay comparable have to be driven from one submission per step. Both are in the fixture's comments.
- **Task 5.2's "would fail against the pre-change code" does not hold as written.** The pre-change solver also handled a capacity change correctly (by rebuilding), so the scenario passes on both revisions; it guards the failure *mode* the deleted rebuild path existed to work around, and it does so bit-exactly. The fixture's fault injection did not reach the counted clear's range either — the clear and the reduce read the same count buffer, and the accumulate recomputes each slot every iteration, so reaching it needs a scenario where a contact stops contributing while staying inside the count.
- **The measurement did not reproduce the proposal's figures as a wall-clock win.** On a 61-shape scene (capacity 18300 slots, on the order of 220 published slots) the counted and full-capacity runs are within noise of each other: the traffic saving is real (roughly 0.5 MB instead of 41 MB of clear+reduce traffic per step) but is a few tens of microseconds against a ~21 ms step that is dominated by fixed per-step costs. The `~890 MB/frame` in the proposal was an estimate and remains one.
