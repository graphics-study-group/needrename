...
Defines the `AssetPath` value type and the scheme-based path resolution performed by `FileSystemDatabase`: `res://` and `builtin://` scheme mounts, normalized generic string paths, and the disk-path resolution boundary between the pure `AssetPath` type and the filesystem-backed database.
