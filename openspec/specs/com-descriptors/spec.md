...
A `CollisionShapeComDescriptor` struct SHALL exist containing: `type` (uint32_t, CollisionShapeType enum value), `feature` (vec4), `local_position` (vec4, COM-local), `local_rotation` (vec4, COM-local), `bound_rigid_body` (uint32_t, INVALID_INDEX if unbound).

The struct SHALL NOT contain `world_position`, `world_rotation`, or filter-list fields. Solvers SHALL compute world pose from COM pose + shape local pose when needed. Collision filter data SHALL be submitted separately via `PhysicsScene::SetShapeFilters`.
