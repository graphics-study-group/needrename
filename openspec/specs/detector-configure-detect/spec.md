...
Define the refactored two-phase API for GPU collision detectors (`ConvexCollisionDetector` and `SpatialHashBroadDetector`): `Configure()` (CPU-side preparation, buffer allocation, pipeline/binding creation) and `Record(vk::CommandBuffer cb)` (direct compute dispatch, no RenderGraph).
