...
Defines the `IPresentProvider` abstraction that separates frame acquisition, copy recording and presentation from `FrameManager`'s frame-completion batch, enabling both windowed (swapchain) and headless (no-op) presentation paths with a single synchronization model.
