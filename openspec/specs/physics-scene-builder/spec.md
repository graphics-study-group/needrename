...
Provide a concise, config-driven helper class (`SceneBuilder`) in the physics example that reduces per-box scene setup boilerplate from ~10 lines to a single function call, using a parent-child GameObject hierarchy to decouple visual mesh scaling from collision geometry.
