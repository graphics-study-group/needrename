...
Provide a concise, config-driven helper class (`SceneBuilder`) in `PhysicsApp` that reduces per-shape scene setup boilerplate to a single function call, using a parent-child GameObject hierarchy to decouple visual mesh scaling from collision geometry. Supports Box, Sphere, and Cylinder shapes with configurable friction, restitution, and color-based material resolution.
