...
- **THEN** it SHALL call `GetAssetRuntime().asset_database->GetAssetPath(GetGUID())` to obtain the in-project `AssetPath`, downcast the database to `FileSystemDatabase`, and pass the `AssetPath` to `ToAbsolutePath`
