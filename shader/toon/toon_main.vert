...
layout(location = 2) out vec3 to_frag_normal_ws;
layout(location = 3) out vec3 to_frag_position_ws;

// Toon shader main pass, which is almost identical to Blinn-Phong vertex shader.
void main() {
    ...
    to_frag_normal_ws = mat3(pc.model) * vertex_normal;
    to_frag_position_ws = vec3(pc.model * vec4(vertex_position.xyz, 1.0));
}