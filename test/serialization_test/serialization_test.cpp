...
int main()
{
    ...
    *ptr_test.m_int_ptr2 = 4654513;
    ...
    for(int i = 0; i < 3; i++)
        assert(dynamic_cast<BaseData *>(array_ptr_test2.m_ptr_array[0][0])->data[i] == 100.0f * i);
    auto inherit_data_ptr3 = dynamic_cast<InheritTest *>(array_ptr_test2.m_ptr_array[1][1]);
    for(int i = 0; i < 3; i++)
