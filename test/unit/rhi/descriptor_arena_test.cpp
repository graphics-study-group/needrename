...
int main() {
...

    {
        ...
        const auto first = arena.AcquireRawSet(layout_ci, "Raw A");
        const auto second = arena.AcquireRawSet(layout_ci, "Raw B");
        ...
        for (size_t i = 0; i < 100; i++) {
            CHECK(arena.AcquireRawSet(layout_ci, "Raw growth") && "a request past one pool's capacity must succeed");
            raw_count++;
        }
        CHECK(
        ...
        for (size_t i = 0; i < 8; i++) {
            ...
            arena.Acquire(layout_ci, 0, true, false, One(f.Buffer(i)));
            f.Tracker().ReportComplete(e);
            arena.AcquireRawSet(layout_ci, "Raw after release");
            raw_count++;
        }
        CHECK(
...

    {
        ...
        const auto first = arena.Acquire(layout_ci, 0, true, false, content_a);
        const auto again = arena.Acquire(layout_ci, 0, true, false, content_a);
        ...
        const auto other = arena.Acquire(layout_ci, 0, true, false, content_b);
        ...
        const auto shifted = arena.Acquire(layout_ci, 0, true, false, One(f.Buffer(1), 16));
        CHECK(!Same(shifted, first) && !Same(shifted, other) && "a differing content mints a distinct set");
        CHECK(arena.GetMintedSetCount() == 3u);
        CHECK(!Same(shifted, arena.Acquire(layout_ci, 0, true, false, content_b)) && "the resident entry is untouched");
        ...
        CHECK(Same(first, arena.Acquire(layout_ci, 0, true, false, content_a)));
        CHECK(Same(other, arena.Acquire(layout_ci, 0, true, false, content_b)));
...

    {
        ...
        const auto e1 = f.Tracker().BeginEpoch();
        const auto set = arena.Acquire(layout_ci, 0, true, false, content);
        f.Tracker().ReportComplete(e1);
        ...
        for (int i = 0; i < 8; i++) {
            const auto e = f.Tracker().BeginEpoch();
            const auto reused = arena.Acquire(layout_ci, 0, true, false, content);
            f.Tracker().ReportComplete(e);
...

    {
        ...
        const auto pinned = arena.Acquire(layout_ci, 0, true, false, pinned_content);
        arena.SetResidentBudget(1);
        ...
        CHECK(Same(pinned, arena.Acquire(layout_ci, 0, true, false, pinned_content)) && "the pinned entry survived");
...

    {
        ...
        auto round = [&](size_t buffer) {
            const auto set = arena.Acquire(layout_ci, 0, true, false, One(f.Buffer(buffer)));
            const auto e = f.Tracker().BeginEpoch();
            f.Tracker().ReportComplete(e);
            return set;
        };
...

    {
        ...
        const auto before_idle = arena.Acquire(layout_ci, 0, true, false, content);
        ...
        CHECK(
            Same(before_idle, arena.Acquire(layout_ci, 0, true, false, content)) && "the entry survives and is reused"
        );
        ...
        CHECK(arena.Acquire(layout_ci, 0, true, false, One(f.Buffer(9))) && "the acquisition succeeds");
...

    {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            const auto e = f.Tracker().BeginEpoch();
            {
                ...
                CHECK(srb.GetDescriptorSet(2, layout, true, false) && "a material-shaped acquisition must succeed");
            }
            f.Tracker().ReportComplete(e);
