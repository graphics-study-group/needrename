#ifndef ANROREFL_REFLECTION_INCLUDED
#define ANROREFL_REFLECTION_INCLUDED
...
namespace AnnoRefl {
    ...
    ANROREFL_API std::shared_ptr<const Type> GetType(const char *name);

    /// @brief Get the Reflection::Type from a name. Return nullptr if the type is not found.
    /// @param name the type name
    /// @return the shared pointer to the Type class. nullptr if the type is not found
    ANROREFL_API std::shared_ptr<const Type> GetType(const std::string &name);
    ...
    std::shared_ptr<const Type> CreateType() {
        if constexpr (std::is_const_v<std::remove_reference_t<T>>) {
            ...
            return std::shared_ptr<const PointerType>(new PointerType(
                GetType<std::remove_pointer_t<T>>(), sizeof(T), PointerType::PointerTypeKind::Raw, DefaultDeleter<T>
            ));
        ...
        if constexpr (std::is_void_v<T>) {
            ...
            return std::shared_ptr<const Type>(
                new Type(typeid(std::remove_const_t<T>).name(), sizeof(T), false, DefaultDeleter<T>)
            );
        }
    }
} // namespace AnnoRefl

#endif // ANROREFL_REFLECTION_INCLUDED
