#ifndef ANROREFL_SERIALIZATION_BASE_INCLUDED
#define ANROREFL_SERIALIZATION_BASE_INCLUDED
...
namespace AnnoRefl {
    ...
    template <typename T>
    concept is_basic_type = std::disjunction<
        std::is_integral<T>,
    ...
    void save_to_archive(const T &value, Archive &archive) {
        ...
        json["%type"] = GetTypeFromObject(value)->GetName();
        json["data"] = value;
    }
    ...
    void save_to_archive(const T &value, Archive &archive) {
        ...
        if (type->IsReflectable()) {
            assert(type->GetTypeKind() == Type::TypeKind::Enum);
            json = std::dynamic_pointer_cast<const EnumType>(type)->to_string(
    ...
    void load_from_archive(T &value, Archive &archive) {
        ...
        if (type->IsReflectable()) {
            ...
            value =
                static_cast<T>(*(std::dynamic_pointer_cast<const EnumType>(type)->from_string(str)));
            return;
        }
...
#endif // ANROREFL_SERIALIZATION_BASE_INCLUDED
