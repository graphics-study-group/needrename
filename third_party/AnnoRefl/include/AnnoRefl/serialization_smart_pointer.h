#ifndef ANROREFL_SERIALIZATION_SMART_POINTER_INCLUDED
#define ANROREFL_SERIALIZATION_SMART_POINTER_INCLUDED
...
namespace AnnoRefl {
    ...
    void load_from_archive(std::unique_ptr<T> &value, Archive &archive) {
        ...
        if (!json.is_null()) {
            ...
            if (json.contains("%type")) type = GetType(json["%type"].get<std::string>());
...
#endif // ANROREFL_SERIALIZATION_SMART_POINTER_INCLUDED
